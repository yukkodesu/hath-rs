...
//! - `Proxy`: reads from a temp file gated on a watch channel written by the download task
...
impl StreamingBody {
    ...
    pub fn new_file(p: FileBodyParams) -> Self {
        Self {
            source: DataSource::File {
                ...
                to_delete: if p.verify { Some(p.path) } else { None },
                cache_handler: p.cache_handler,
            },
            offset: 0,
    ...
    fn poll_frame(
        ...
        loop {
            ...
            if matches!(self.source, DataSource::Proxy { .. }) {
                ...
                let need = TCP_PACKET_SIZE.min(current_total - current_offset);

                enum ProxyGate {
                    Ready,
                    WaitAndContinue,
                    TerminalFailure(BodyIncompleteReason),
                }
                ...
                } = &mut self.source
                {
                    ...
                    let (write_offset, fail_incomplete) = {
                        let state = watch_rx.borrow_and_update();
                        match *state {
                            DownloadState::InProgress(n) => (n, None),
                    ...
                    let have_enough = write_offset >= (current_offset + need) as u64;

                    if have_enough {
                        *wait_fut = None;
                        ProxyGate::Ready
                    } else if let Some(incomplete) = fail_incomplete {
                        ProxyGate::TerminalFailure(incomplete)
                    } else {
                ...
                match gate {
                    ProxyGate::Ready => {}
                    ProxyGate::WaitAndContinue => continue,
                    ProxyGate::TerminalFailure(incomplete) => {
                        // Log with temp_file path for context.
                        if let DataSource::Proxy { temp_file, .. } = &self.source {
                            tracing::warn!(
                                "Proxy body: download failed ({}) for {} (offset={} total={})",
                                incomplete.as_str(),
                                temp_file.display(),
                                current_offset,
                                current_total,
                        ...
                        self.finish_with(Some(incomplete));
                        return Poll::Ready(None);
                    }
                }
            }
            ...
            let result = match &mut self.source {
                ...
                DataSource::Proxy { file, file_buf, .. } => {
                    ...
                    // fill_file_buffer will not block waiting for data.
                    match Self::fill_file_buffer(file, file_buf, actual_size) {
...
mod tests {
    ...
    fn file_body_skips_integrity_check_when_send_is_partial() {
        ...
        let mut body = StreamingBody::new_file(FileBodyParams {
            file,
            path: path.clone(),
            total_size: "complete body".len(),
    ...
    async fn proxy_body_records_incomplete_when_download_fails() {
        ...
        let _ = tx.send(DownloadState::Failed(DownloadFailReason::NetworkError));
